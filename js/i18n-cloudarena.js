(() => {
    ...
    const CATALOG_URL = (locale) => `/locales/${locale}/cloudarena-literals.json`;
    ...
    async function loadCatalogs() {
        ...
        if (!ptResponse.ok || !enResponse.ok) {
            throw new Error(`CloudArena literal catalogs unavailable: PT=${ptResponse.status}, EN=${enResponse.status}`);
        }
        const [pt, en] = await Promise.all([ptResponse.json(), enResponse.json()]);
        const ptMap = pt.cloudarenaLiterals || {};
        const enMap = en.cloudarenaLiterals || {};
        ...
        if (ptKeys.length !== enKeys.length || ptKeys.some((key, index) => key !== enKeys[index])) {
            throw new Error('CloudArena PT/EN literal catalogs are out of parity');
        }
    ...
    async function apply(root = document) {
        ...
        try {
            ...
            document.documentElement.dataset.cloudarenaLocale = locale;
            applyNode(root, buildMap(locale), locale);
        } catch (error) {
            console.error('[CloudArena i18n]', error);
        } finally {
            applying = false;
        }
    }
