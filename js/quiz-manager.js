...
class SocketClient {
    ...
    connect() {
        ...
        this.socket.onAny((event, ...args) => {
            // Log apenas para eventos não são de heartbeat/ping
    ...
    off(event, callback) {
        if (this.eventHandlers.has(event)) {
            ...
            if (callback) {
                ...
                // Se não especificar callback, remove todos
    ...
    // Criar sala (host) - CORRIGIDO para incluir creatorId
    ...
    getRoomState(roomId, callback) {
        this.emit('get-room-state', { roomId }, callback);
    }
